    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (filename.endsWith(".xml")) {
            fs.readFile(fullpath, (err, data) => {
                ...
                let startReg = /<statement name/g, result, startIndices = [];
                while ((result = startReg.exec(input))) {
                    startIndices.push(result.index);
                }
                let endReg = /<\/statement>/g, endIndices = [];
...
function populateBlockDict(blocks, dict) {
    ...
    for(var i = 0; i < blocks.length; i++) {
        ...
        islandCount += (blockType.includes("when") || blockType.includes("procedures_def")) ? 1 : 0;
    }
    return islandCount;
}
...
function getMaxDepth(input, startIndices, endIndices) {
    ...
    while (e_i < endIndices.length) {
        ...
        if (s_i < startIndices.length || e_i < (endIndices.length)) {
            ...
            input = input.substring(0, start) + subInput.replace("<next>", "<abcd>") + input.substring(currEnd); 
            e_i++;
        }
    }
    return maxDepth;
