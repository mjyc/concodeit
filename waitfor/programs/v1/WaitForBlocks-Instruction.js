...
while (!correctDir) {
  ...
  await waitForOne(promisify2(async cb => {
    await waitForSpecificEvent(String("humanFaceLookingAtRight")); if (_stop["Irv,p~4(.Y8.P^U!!nr9"]) return;
    correctDir = true; if (_stop["Irv,p~4(.Y8.P^U!!nr9"]) return;
    cb(null, null);
  })(), promisify2(async cb => {
    await waitForSpecificEvent(String("humanFaceLookingAtLeft")); if (_stop["Irv,p~4(.Y8.P^U!!nr9"]) return;
...
while (!correctDir) {
  ...
  await waitForOne(promisify2(async cb => {
    await waitForSpecificEvent(String("humanFaceLookingAtLeft")); if (_stop["vv*Gx05?{DAHn%x-,b=x"]) return;
    correctDir = true; if (_stop["vv*Gx05?{DAHn%x-,b=x"]) return;
    cb(null, null);
  })(), promisify2(async cb => {
    await waitForSpecificEvent(String("humanFaceLookingAtRight")); if (_stop["vv*Gx05?{DAHn%x-,b=x"]) return;
